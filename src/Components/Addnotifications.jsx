...
const NotificationManager = () => {
  ...
  const fetchNotifications = async () => {
    try {
      const response = await axios.get('http://mazedakhale.in:3000/notifications'); // Adjust API URL as needed
  ...
  const handleAddNotification = async () => {
    try {
      ...
      await axios.post('http://mazedakhale.in:3000/notifications', payload);
      fetchNotifications();
  ...
  const handleDelete = async (id) => {
    ...
    if (confirmDelete.isConfirmed) {
      ...
      axios
        .delete(`http://mazedakhale.in:3000/notifications/${id}`)
        .then(() => {
  ...
  const handleSaveEdit = async (id) => {
    try {
      await axios.put(`http://mazedakhale.in:3000/notifications/${id}`, editData);
      setEditingId(null);
      fetchNotifications();
    } catch (err) {
  ...
  const handleToggleStatus = async (id, currentStatus) => {
    try {
      ...
      await axios.patch(`http://mazedakhale.in:3000/notifications/status/${id}`, { notification_status: newStatus });
      fetchNotifications();
    } catch (err) {
