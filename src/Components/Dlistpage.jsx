...
const DlistPage = () => {
  ...
  useEffect(() => {
    if (categoryId && subcategoryId && distributorId) {
      const DOCUMENTS_API_URL = `https://65.2.172.92:3000/documents/${categoryId}/${subcategoryId}?distributorId=${distributorId}`;
  ...
  const handleUploadCertificate = async (documentId) => {
    ...
    try {
      await axios.post(
        "https://65.2.172.92:3000/certificates/upload",
        formData,
        {
  ...
  const handleDownloadCertificate = async (documentId, name) => {
    ...
    try {
      const response = await axios.get(`https://65.2.172.92:3000/certificates/${certificate.certificate_id}`);
