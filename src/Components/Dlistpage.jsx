...
const DlistPage = () => {
  ...
  useEffect(() => {
    if (categoryId) {
      axios
        .get(`http://72.60.206.65:3000/categories/${categoryId}`)
        .then((res) => setCategoryName(res.data.category_name))
        .catch((err) => console.error("Error loading category:", err));
    }
    if (subcategoryId) {
      axios
        .get(`http://72.60.206.65:3000/subcategories/${subcategoryId}`)
  ...
  useEffect(() => {
    ...
    const url = `http://72.60.206.65:3000/documents/${categoryId}/${subcategoryId}?distributorId=${distributorId}`;
    axios
      .get(url)
      .then((resp) => {
