...
const AddDistributorModal = ({ isOpen, onClose, fetchDistributors }) => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const response = await axios.post("https://https://mazedakhale.in/users/register", formDataToSend, {
                headers: {
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center">
            <div className="bg-white p-6 rounded-lg w-1/3">
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                    ...
                    <input type="text" name="shopAddress" placeholder="Shop Address (Optional)" className="w-full p-2 border rounded" onChange={handleChange} />

                    <div>
                        <label className="block font-medium">Upload Aadhar Card</label>
                        <input type="file" accept=".pdf,.jpg,.jpeg,.png" onChange={(e) => handleFileChange(e, "aadharCard")} className="w-full p-2 border rounded" required />
                    ...
                    <div>
                        ...
                        <input type="file" accept=".pdf,.jpg,.jpeg,.png" onChange={(e) => handleFileChange(e, "panCard")} className="w-full p-2 border rounded" required />
                    ...
                    <div className="flex justify-end space-x-2">
                        ...
                        <button type="submit" className="bg-blue-500 text-white px-4 py-2 rounded">Add Distributor</button>
                    </div>
                </form>
            </div>
        </div>
