...
const FieldNames = () => {
  ...
  const fetchFields = async () => {
    try {
      const response = await axios.get("http://13.201.37.154:3000/field-names");
  ...
  const handleDelete = async (id) => {
    ...
    if (confirmDelete.isConfirmed) {
      ...
      axios
        .delete(`http://13.201.37.154:3000/field-names/${id}`)
        .then(() => {
  ...
  const handleSave = async (id) => {
    try {
      await axios.patch(`http://13.201.37.154:3000/field-names/${id}`, { document_fields: editableField });
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      await axios.post("http://13.201.37.154:3000/field-names", formData);
