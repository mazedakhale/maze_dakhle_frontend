...
const FieldNames = () => {
  ...
  const fetchFields = async () => {
    try {
      const response = await axios.get("https://65.2.172.92:3000/field-names");
  ...
  const handleDelete = async (id) => {
    ...
    if (confirmDelete.isConfirmed) {
      ...
      axios
        .delete(`https://65.2.172.92:3000/field-names/${id}`)
        .then(() => {
  ...
  const handleSave = async (id) => {
    try {
      await axios.patch(`https://65.2.172.92:3000/field-names/${id}`, { document_fields: editableField });
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      await axios.post("https://65.2.172.92:3000/field-names", formData);
