...
const FieldNames = () => {
  ...
  const fetchFields = async () => {
    try {
      const response = await axios.get("http://mazedakhale.in:3000/field-names");
  ...
  const fetchSubcategories = async (categoryId) => {
    ...
    try {
      const response = await axios.get(`http://mazedakhale.in:3000/subcategories/category/${categoryId}`);
  ...
  const handleDelete = async (id) => {
    ...
    if (confirmDelete.isConfirmed) {
      ...
      axios
        .delete(`http://mazedakhale.in:3000/field-names/${id}`)
        .then(() => {
  ...
  const handleSave = async (id) => {
    try {
      await axios.patch(`http://mazedakhale.in:3000/field-names/${id}`, { document_fields: editableField });
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      await axios.post("http://mazedakhale.in:3000/field-names", formData);
