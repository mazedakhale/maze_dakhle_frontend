...
export default function PricingPage() {
    ...
    const pickCategory = (cat) => {
        setSelectedCat(cat);
        axios.get(API_SUBCATEGORIES)
            .then(({ data }) =>
                setSubcategories(data.filter(
                    sub => sub.category.category_id === cat.category_id
                ))
            )
            .catch(console.error);
    };
            ...
            <section className="bg-white py-16">
                <div className="max-w-4xl mx-auto text-center px-6">
                    ...
                    <p className="text-gray-600 mb-8">
                        Maze Dakhale — Your Trusted Partner for Online Government Applications
                    </p>
                    <div className="flex flex-wrap justify-center gap-4">
                        {categories.map(cat => {
                            const Icon = categoryIconMapping[cat.category_name] || FaCertificate;
