...
const Emplist = () => {
    ...
    useEffect(() => {
        ...
        axios
            .get("http://13.201.37.154:3000/documents/list")
            .then((response) => {
        ...
        // Fetch certificates
        axios
            .get("http://13.201.37.154:3000/certificates")
            .then((response) => setCertificates(response.data))
            .catch((error) => console.error("Error fetching certificates:", error));

        // Fetch users
        axios
            .get("http://13.201.37.154:3000/users/register")
    ...
    const handleVerifyDocument = async (documentId) => {
        try {
            ...
            if (verifiedBy) {
                ...
                await axios.put(
                    `http://13.201.37.154:3000/documents/verify/${documentId}`,
                    { verifiedBy }
                );
    ...
    const handleUpdateStatus = async (documentId, newStatus) => {
        try {
            await axios.put(`http://13.201.37.154:3000/documents/update-status/${documentId}`, {
                status: newStatus,
            });
            setDocuments((prev) =>
                prev.map((doc) =>
    ...
    const filteredDocuments = documents
        ...
        .filter((doc) => {
            ...
            return Array.isArray(doc.verified_by) && doc.verified_by.includes(userId.toString());
        });
    ...
    const getDistributorName = (distributorId) => {
        ...
        const distributor = users.find((user) => String(user.user_id) === String(distributorId));
    ...
    const handleViewInvoice = async (documentId, categoryId, subcategoryId) => {
        try {
            ...
            const alreadyVerified = document && document.verified_by &&
    ...
    const handleViewCertificate = async (documentId) => {
        ...
        try {
            const response = await axios.get(`http://13.201.37.154:3000/certificates/${certificateId}`);
    ...
    const getUserNameById = (userId) => {
        const user = users.find((user) => user.user_id.toString() === userId.toString());
                        ...
                        <tbody>
                            {filteredDocuments.map((doc, index) => (
                                ...
                                >
                                                ...
                                                <div className="flex flex-col">
                                                    ...
                                                    {doc.verified_at && (
                                                        <span className="text-xs text-gray-600 mt-1">
                                                            {new Date(doc.verified_at).toLocaleString('en-US', {
                                                                year: 'numeric',
                                                                month: '2-digit',
                                                                day: '2-digit',
                                                                hour: '2-digit',
                                    ...
                                    <td>{getUserNameById(doc.verified_by?.[0])}</td> {/* Assuming first verifier's ID */}
