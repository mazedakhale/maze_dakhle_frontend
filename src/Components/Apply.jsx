...
const Apply = () => {
  ...
  useEffect(() => {
    if (formData.category_id && formData.subcategory_id) {
      axios
        .get(`http://13.201.37.154:3000/required-documents/${formData.category_id}/${formData.subcategory_id}`)
        .then((response) => {
  ...
  useEffect(() => {
    if (formData.category_id && formData.subcategory_id) {
      axios
        .get(`http://13.201.37.154:3000/field-names/${formData.category_id}/${formData.subcategory_id}`)
        .then((response) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post(
        "http://13.201.37.154:3000/documents/upload",
        formDataToSend,
        {
