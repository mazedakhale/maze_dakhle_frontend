...
const Employee = () => {
    ...
    const fetchDocuments = async () => {
        setIsLoading(true);
        try {
            const response = await axios.get("https://https://mazedakhale.in/employee");
            setDocuments(response.data);
        } catch (error) {
            console.error("Error fetching documents:", error);
            Swal.fire("Error", "Failed to fetch documents. Server might be down.", "error");
        } finally {
            setIsLoading(false);
        }
    };
    ...
    const fetchCategories = async () => {
        try {
            const response = await axios.get("https://https://mazedakhale.in/categories");
    ...
    const fetchEmployees = async () => {
        try {
            ...
            const response = await axios.get("https://https://mazedakhale.in/users/employee");
    ...
    const getEmployeeName = (userId) => {
        const employee = employees.find(emp => emp.user_id === parseInt(userId) || emp.user_id === userId);
        return employee ? employee.name : userId;
    };

    const handleCategoryChange = async (e) => {
        const selectedCategoryId = e.target.value;
        setFormData({ ...formData, category_id: selectedCategoryId, subcategory_ids: [] });

        if (selectedCategoryId) {
            try {
                const response = await axios.get(`https://https://mazedakhale.in/subcategories/category/${selectedCategoryId}`);
    ...
    const handleSubcategoryChange = (subcategoryId) => {
        setFormData(prevData => {
            ...
            if (prevData.subcategory_ids.includes(subcategoryId)) {
                ...
                return {
                    ...prevData,
                    subcategory_ids: prevData.subcategory_ids.filter(id => id !== subcategoryId)
                };
            } else {
    ...
    const handleDelete = async (id) => {
        ...
        if (confirmDelete.isConfirmed) {
            try {
                ...
                await axios.delete(`https://https://mazedakhale.in/employee/${id}`);
    ...
    const handleEdit = async (groupedDoc) => {
        ...
        try {
            const response = await axios.get(`https://https://mazedakhale.in/subcategories/category/${groupedDoc.category_id}`);
            setSubcategories(response.data);
        } catch (error) {
            console.error("Error fetching subcategories:", error);
            setSubcategories([]);
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (editId) {
                ...
                await axios.put(`https://https://mazedakhale.in/employee/${editId}`, dataToSend);
            } else {
                // For create, use the POST endpoint with the new API format
                await axios.post("https://https://mazedakhale.in/employee", dataToSend);
            }
                                        ...
                                        <div key={subcategory.subcategory_id} className="mb-2">
                                            <label className="inline-flex items-center">
                                                <input
                                                    ...
                                                    checked={formData.subcategory_ids.includes(subcategory.subcategory_id.toString())}
                                                    onChange={() => handleSubcategoryChange(subcategory.subcategory_id.toString())}
                                                    className="form-checkbox h-5 w-5 text-orange-500"
                                                />
                                                <span className="ml-2">{subcategory.subcategory_name}</span>
                                            </label>
                                        </div>
                                    ))
                                ) : (
                    ...
                    {isLoading ? (
                        ...
                        <table className="w-full border border-[#776D6DA8] text-sm bg-white shadow-md rounded-md">
                            <thead className="bg-[#F58A3B14] border-b-2 border-[#776D6DA8]">
                                <tr>
                                    {["Category", "Subcategories", "Employee", "Actions"].map((header, index) => (
                                        <th key={`header-${index}`} className="px-4 py-3 border border-[#776D6DA8] text-black font-semibold text-center">
                                            {header}
                                        </th>
                                    ))}
                                </tr>
                                ...
                                {documents.length > 0 ? (
                                    ...
                                    Object.values(documents.reduce((acc, doc) => {
                                        ...
                                        if (!acc[key]) {
                                            ...
                                            if (!acc[key].subcategory_ids.includes(doc.subcategory_id)) {
                                        ...
                                        >
                                            <td className="px-4 py-3 border border-[#776D6DA8] text-center">
                                                {categories.find(c => c.category_id === groupedDoc.category_id)?.category_name || groupedDoc.category_id}
                                            </td>
                                            <td className="px-4 py-3 border border-[#776D6DA8] text-center">
