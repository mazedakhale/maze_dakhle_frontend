...
const ErrorRequests = () => {
  ...
  const fetchErrorRequests = async (distributorId) => {
    try {
      ...
      const response = await axios.get(
        ` http://65.2.172.92:3000/request-errors/distributor/${distributorId}`
      );
  ...
  const handleRejectStatus = async (requestId) => {
    ...
    try {
      ...
      const response = await axios.patch(` http://65.2.172.92:3000/request-errors/update-status/${requestId}`, {
        request_status: "Distributor Rejected",
  ...
  const handleUploadCertificate = async (documentId, requestId) => {
    ...
    try {
      const uploadResponse = await axios.patch(
        `http://65.2.172.92:3000/certificates/update/${documentId}`,
        formData,
        {
          headers: {
      ...
      const updateStatusResponse = await axios.patch(
        `http://65.2.172.92:3000/request-errors/update-status/${requestId}`,
