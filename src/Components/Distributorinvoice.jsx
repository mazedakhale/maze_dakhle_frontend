...
const InvoicePage = () => {
  ...
  const handleUploadReceipt = async () => {
    ...
    try {
      ...
      const response = await axios.post(
        `http://mazedakhale.in:3000/documents/upload-receipt/${documentId}`,
        formData,
        {
          headers: {
  ...
  const handleDownloadAllDocuments = async () => {
    try {
      ...
      const response = await axios.get(`http://mazedakhale.in:3000/download/${documentId}`, {
  ...
  const handleAssignDistributor = async (distributorId) => {
    ...
    try {
      await axios.put(`http://mazedakhale.in:3000/documents/assign-distributor/${documentId}`, {
  ...
  const fetchDocumentData = useCallback(async () => {
    try {
      const response = await axios.get(`http://mazedakhale.in:3000/singledocument/documentby/${documentId}`);
      ...
      if (category && subcategory) {
        const fieldNamesResponse = await axios.get(`http://mazedakhale.in:3000/field-names/${category}/${subcategory}`);
  ...
  const DownloadAllDocuments = async () => {
    try {
      ...
      const response = await axios.get(
        `http://mazedakhale.in:3000/download/all/${documentId}`,
        {
          responseType: "blob",
