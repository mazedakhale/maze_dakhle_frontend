...
const EmployeeReceiptList = () => {
  ...
  const fetchDocuments = async () => {
    setLoading(true);
    try {
      // Fetch assigned documents from the same API as admin
      const response = await axios.get(`${API_BASE_URL}/documents/assigned-list`, {
        headers: {
          Authorization: `Bearer ${localStorage.getItem("token")}`
        },
        timeout: 120000
  ...
  // Filter documents based on search query, status filter, and employee assignments
  const filteredDocuments = documents
    .filter(isDocumentAssignedToEmployee) // Only show assigned documents
    .filter((doc) => doc.status === "Sent") // Only documents with Sent status (waiting for receipt verification)
    .filter((doc) => doc.receipt_url) // Only documents with receipts
    .filter((doc) =>
  ...
  const handleCompleteReceipt = async (documentId, applicationId) => {
    try {
      const result = await Swal.fire({
        title: "Verify Receipt",
        text: `Are you sure you want to verify receipt for application ${applicationId}?`,
        icon: "question",
      ...
      if (result.isConfirmed) {
        ...
        setDocuments((prev) =>
          prev.map((doc) =>
            doc.document_id === documentId ? { ...doc, status: "Received" } : doc
          )
        );

        Swal.fire("Success!", "Receipt has been verified successfully.", "success");
      }
    } catch (error) {
      console.error("Error verifying receipt:", error);
      Swal.fire("Error", "Failed to verify receipt. Please try again.", "error");
    }
  };

  // Handle reject receipt
  const handleRejectReceipt = async (documentId, applicationId) => {
    try {
      const { value: rejectionReason } = await Swal.fire({
        title: "Reject Receipt",
        input: "text",
        inputLabel: "Rejection Reason",
        inputPlaceholder: "Enter the reason for rejecting this receipt...",
        showCancelButton: true,
      ...
      Swal.fire("Error", "Failed to reject receipt. Please try again.", "error");
    }
  };
      ...
      <div className="w-[90%] max-w-6xl bg-white shadow-lg rounded-lg">
        <div className="relative border-t-4 border-orange-400 bg-[#F4F4F4] p-4 rounded-t-lg">
          <h2 className="text-2xl font-bold text-gray-800 text-center">
            Receipt List - Assigned Documents
          </h2>
          <button
            onClick={() => navigate(-1)}
          ...
          <table className="w-full border-collapse border border-gray-300">
            ...
            <tbody>
              {filteredDocuments.length > 0 ? (
                    ...
                    <td className="border p-2 text-center">
                      <div className="flex justify-center space-x-2">
                        {doc.status === "Sent" && (
                          <>
                            <button
                              onClick={() => handleCompleteReceipt(doc.document_id, doc.application_id)}
                            ...
                            <button
                              onClick={() => handleRejectReceipt(doc.document_id, doc.application_id)}
                ...
                <tr>
                  <td colSpan="11" className="text-center py-10 bg-gray-50">
                    <FaRegFileAlt className="text-4xl text-gray-400 mx-auto mb-2" />
                    <p className="text-gray-500">
                      No receipts found for your assigned categories.
                    </p>
                  </td>
                </tr>
              )}
