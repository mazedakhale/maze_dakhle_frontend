...
const Login = () => {
  ...
  useEffect(() => {
    const fetchCategories = async () => {
      try {
        const response = await fetch("http://65.2.172.92:3000/categories");
        if (!response.ok) {
      ...
      try {
        ...
        for (const category of categories) {
          const response = await fetch(
            `http://65.2.172.92:3000/subcategories/category/${category.category_id}`
          );
          if (response.ok) {
            const data = await response.json();
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const response = await fetch("http://65.2.172.92:3000/users/login", {
        method: "POST",
