...
import { FaUsers, FaStore, FaFileAlt, FaCogs, FaBoxes, FaClipboardList, FaFilePdf } from "react-icons/fa";
...
const Adashinner = () => {
  ...
  const staticCards = [
    { id: "users", icon: FaUsers, title: "Users", color: "bg-[#FE1008]", linkTo: "/Userlist" }, // 🔴 Red
    { id: "distributors", icon: FaStore, title: "Distributors", color: "bg-[#1D4ED8]", linkTo: "/Distributorlistonly" }, // 🔵 Blue
    { id: "documents", icon: FaFileAlt, title: "Documents", color: "bg-[#FACC15]", linkTo: "/Verifydocumentshistory" }, // 🟡 Yellow
    { id: "categories", icon: FaCogs, title: "Categories", color: "bg-[#22C55E]", linkTo: null }, // 🟢 Green
    { id: "subcategories", icon: FaBoxes, title: "Subcategories", color: "bg-[#9333EA]", linkTo: null }, // 🟣 Purple
    { id: "recent", icon: FaClipboardList, title: "View Recent Applications", color: "bg-[#F97316]", onClick: () => navigate("/Recentapplications") }, // 🟠 Orange
    { id: "sent", icon: FaClipboardList, title: "Sent Applications", color: "bg-[#E11D48]", onClick: () => navigate("/Assigndistributorlist") }, // 🟥 Dark Pink/Red
    { id: "received", icon: FaClipboardList, title: "Received Documents", color: "bg-[#0EA5E9]", onClick: () => navigate("/Uploadeddocuments") }, // 🟦 Light Blue
    { id: "completed", icon: FaClipboardList, title: "Completed Tasks", color: "bg-[#16A34A]", onClick: () => navigate("/Verifydocumentshistory") } // 🟢 Dark Green
  ];
  ...
  useEffect(() => {
    ...
    const fetchAllData = async () => {
      try {
        ...
        const promises = [
          fetch("https://mazedakhale.in:3000/statistics/counts").then(res => res.json()),
          axios.get("https://mazedakhale.in:3000/categories"),
          axios.get("https://mazedakhale.in:3000/subcategories"),
          axios.get("https://mazedakhale.in:3000/statistics/cscounts")

        ];

        // Wait for all promises to resolve
        const [countsData, categoriesResponse, subcategoriesResponse, csCountsResponse] = await Promise.all(promises);
  ...
  const pieChartData = useMemo(() => ({
    ...
    datasets: [
      {
        data: [
          counts?.documentStatus?.find(({ status }) => status === "Approved")?.count || 0,
          counts?.documentStatus?.find(({ status }) => status === "Pending")?.count || 0,
          counts?.documentStatus?.find(({ status }) => status === "Rejected")?.count || 0,
          counts?.documentStatus?.find(({ status }) => status === "Completed")?.count || 0,
        ],
          ...
          return (
            ...
            >
              ...
              <div className={`${card.color} flex items-center justify-center w-1/4 h-full rounded-l-lg`}>
                <card.icon className="text-white text-5xl" />  {/* Increased icon size */}
              </div>
              ...
              <div className="w-3/4 flex flex-col justify-center pl-3">
                ...
                {count !== null && <p className="text-3xl font-bold">{count}</p>}  {/* Bigger count */}
              </div>
            </div>
          );
        })}
          ...
          categories.map((category) => {
            ...
            return (
              <div key={category.category_id} className="mb-8 mt-10">
                ...
                <div
                  className="flex w-1/2 rounded-lg shadow-[0px_2px_4px_rgba(0,0,0,0.4)] cursor-pointer transition-all duration-300 overflow-hidden" onClick={() => fetchSubcategories(category.category_id, category.category_name)}
                >
                  {/* ✅ Icon Section */}
                  ...
                  <div className="flex-1 bg-gray-100 hover:bg-orange-200 p-4 flex justify-between items-center">
                    ...
                    <span className="text-sm text-gray-600">Pending: {categoryCount}</span>
                  </div>
                </div>
                    ...
                    subcategoriesMap[category.category_id].map((subcategory) => {
                      ...
                      return (
                        <div
                          ...
                          onClick={() => handleSubcategorySelect(category.category_id, category.category_name, subcategory.subcategory_id, subcategory.subcategory_name)}
                        >
                          {/* ✅ Icon Section */}
                          <div className="mr-2">
                          ...
                          <div className="flex-1">
                            <h4 className="font-semibold text-black">{subcategory.subcategory_name}</h4>
                            <span className="text-sm text-gray-600">({subcategoryCount})</span>
                          </div>
                        </div>
                      );
                    })
