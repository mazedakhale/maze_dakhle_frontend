...
const FeedbackList = () => {
    ...
    const handleDelete = async (id) => {
        ...
        setFeedbackList((prev) => prev.filter((f) => f.feedback_feedback_id !== id));
        try {
                ...
                <div className="p-6 overflow-x-auto">
                    <table className="w-full bg-white text-sm border border-[#776D6DA8] rounded-md shadow-md">
                        <thead className="bg-[#F58A3B14] border-b-2 border-[#776D6DA8]">
                            <tr>
                                {['Name', 'Role', 'Comment', 'Rating', 'Update', 'Action'].map((h, i) => (
                                    <th key={i} className="px-4 py-3 border text-center font-semibold">{h}</th>
                                ))}
                            </tr>
                        </thead>
                        <tbody>
                            {feedbackList.length ? feedbackList.map((fb, idx) => (
                                <tr key={fb.feedback_feedback_id} className={`${idx % 2 === 0 ? 'bg-white' : 'bg-[#F58A3B14]'} hover:bg-orange-100`}>
                                    <td className="px-4 py-3 border text-center">{fb.users_name}</td>
                                    <td className="px-4 py-3 border text-center">{fb.users_role}</td>
                                    <td className="px-4 py-3 border text-center">{fb.feedback_comment}</td>
                                    <td className="px-4 py-3 border text-center">
                                        {[...Array(fb.feedback_rating)].map((_, i) => <StarIcon key={i} color="warning" />)}
                                    </td>
                                    <td className="px-4 py-3 border text-center">
                                        <button
                                            className={`px-2 py-1 rounded ${fb.status === 1 ? 'bg-green-500 text-white' : 'bg-gray-200'}`}
                                ...
                                <tr>
                                    <td colSpan={6} className="px-4 py-3 border text-center">No feedback found.</td>
                                </tr>
                            )}
                        </tbody>
                    </table>
