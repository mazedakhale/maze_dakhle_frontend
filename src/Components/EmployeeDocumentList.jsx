...
const EmployeeDocumentList = () => {
    ...
    useEffect(() => {
        ...
        if (categoryId && subcategoryId && token) {
            ...
            if (!categoryId || !subcategoryId) {
                Swal.fire("Error", "Category or subcategory information is missing", "error");
            } else if (!token) {
    ...
    const fetchDocuments = async (categoryId, subcategoryId, userId, userRole) => {
        try {
            // Check if user role is Admin or Employee
            if (userRole === "Admin" || userRole === "Employee") {
                // Use the API endpoint
                const response = await axios.get(
                    `https://vm.q1prh3wrjc0aw.ap-south-1.cs.amazonlightsail.com/documents/category-docs/${categoryId}/${subcategoryId}/${userId}`
                );
    ...
    const handleUnauthorized = () => {
        setDocuments([]);
        Swal.fire("Access Denied", "You don't have permission to view these documents", "error");
    ...
    const handleViewCertificate = async (documentId) => {
        try {
            const response = await axios.get(`https://vm.q1prh3wrjc0aw.ap-south-1.cs.amazonlightsail.com/certificates/by-document/${documentId}`);
            ...
            <div className="bg-white shadow-lg rounded-lg border border-gray-300">
                ...
                <div className="border-t-4 border-orange-400 bg-[#F4F4F4] text-center p-4 rounded-t-lg">
                    <h2 className="text-xl font-bold text-center text-gray-800">
                        {categoryInfo.categoryName} &gt; {categoryInfo.subcategoryName} Documents
                    </h2>
                </div>
                        ...
                        {filteredDocuments.length > 0 ? (
                            filteredDocuments.map((doc, index) => (
                                ...
                                >
                                    ...
                                    <td className="border p-2 text-center">{doc.application_id || doc.document_id}</td>
                                    <td className="px-4 py-2 border text-sm">
                                        {doc?.document_fields ? (
                                            ...
                                            <p className="text-gray-500">{doc.document_name || "No name available"}</p>
                                        )}
                                    </td>
                                    <td className="border p-2">
                                        {new Date(doc.upload_date || doc.uploaded_at).toLocaleString('en-US', {
                                            year: 'numeric',
                                            month: '2-digit',
                                            day: '2-digit',
                                            hour: '2-digit',
                                    ...
                                    <td className="border p-2">{doc.category_name || categoryInfo.categoryName}</td>
                                    <td className="border p-2">{doc.subcategory_name || categoryInfo.subcategoryName}</td>
                                    ...
                                    <td className="border p-2">
                                        <button
                                            ...
                                            onClick={() => handleViewInvoice(doc.document_id || doc.id, categoryInfo.categoryId, categoryInfo.subcategoryId)}
                                        >
                                    ...
                                    <td className="border p-2">
                                        {doc.receipt_url ? (
                                            <button
                                                ...
                                                onClick={() => handleDownloadReceipt(doc.receipt_url, doc.document_name || "document")}
                                            >
                                    ...
                                    <td className="border p-2">
                                        {doc.certificate_id || doc.has_certificate ? (
                                            <button
                                                ...
                                                onClick={() => handleViewCertificate(doc.document_id || doc.id)}
                                            >
                            ...
                            <tr>
                                <td colSpan="14" className="text-center py-10 bg-gray-50">
                                    ...
                                    <p className="text-gray-500">No documents found for this category and subcategory.</p>
                                </td>
                            </tr>
                        )}
                    </tbody>
