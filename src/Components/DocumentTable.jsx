...
const DocumentTable = () => {
  ...
  const fetchDocuments = async () => {
    try {
      const response = await axios.get("http://mazedakhale.in:3000/document-types");
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editingDoc) {
        ...
        await axios.put(
          `http://mazedakhale.in:3000/document-types/${editingDoc.doc_type_id}`,
          formData
        );
      } else {
        // Add new document
        await axios.post("http://mazedakhale.in:3000/document-types/", formData);
      }
  ...
  const handleDelete = async (id) => {
    ...
    if (confirmDelete.isConfirmed) {
      try {
        await axios.delete(`http://mazedakhale.in:3000/document-types/${id}`);
        fetchDocuments(); // Refresh the document list
