...
const DocumentTable = () => {
  ...
  const fetchDocuments = async () => {
    try {
      const { data } = await axios.get("http://72.60.206.65:3000/document-types");
      setDocuments(data);
    } catch (error) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editingDoc) {
        await axios.put(
          `http://72.60.206.65:3000/document-types/${editingDoc.doc_type_id}`,
          formData
        );
      } else {
        await axios.post("http://72.60.206.65:3000/document-types/", formData);
      }
  ...
  const handleDelete = async (id) => {
    ...
    if (res.isConfirmed) {
      try {
        await axios.delete(`http://72.60.206.65:3000/document-types/${id}`);
        fetchDocuments();
