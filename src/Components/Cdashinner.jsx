...
const CustomerDashboard = () => {
  ...
  const API_BASE_URL = "http://65.2.172.92:3000/categories";
  const SUBCATEGORIES_API_URL = "http://65.2.172.92:3000/subcategories";
  ...
  useEffect(() => {
    ...
    axios.get(`http://65.2.172.92:3000/userdashboard/total-applied/${userId}`)
    ...
    axios.get(`http://65.2.172.92:3000/userdashboard/total-completed/${userId}`)
      .then((res) => setCompletedCount(res.data.totalCompleted))
      .catch((err) => console.error("Error fetching total completed:", err));

    // Fetch category counts
    axios.get(`http://65.2.172.92:3000/userdashboard/category-counts/${userId}`)
      .then((res) => {
    ...
    axios.get(`http://65.2.172.92:3000/userdashboard/status-count/${userId}`)
      .then((res) => {
    ...
    axios.get("http://65.2.172.92:3000/notifications/active")
  ...
  const fetchRequiredDocuments = async (categoryId, subcategoryId) => {
    try {
      const response = await axios.get(
        `http://65.2.172.92:3000/required-documents/${categoryId}/${subcategoryId}`
      );
