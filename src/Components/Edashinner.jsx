...
import { FaUsers, FaStore, FaFileAlt, FaCogs, FaBoxes, FaClipboardList } from "react-icons/fa";
...
const Edashinner = () => {
    ...
    const staticCards = [
        { id: "users", icon: FaUsers, title: "Users", color: "bg-[#FE1008]", linkTo: "/Userlist" },
        { id: "distributors", icon: FaStore, title: "Distributors", color: "bg-[#1D4ED8]", linkTo: "/Distributorlistonly" },
        { id: "documents", icon: FaFileAlt, title: "Documents", color: "bg-[#FACC15]", linkTo: "/Verifydocumentshistory" },
        { id: "categories", icon: FaCogs, title: "Categories", color: "bg-[#22C55E]", linkTo: null },
        { id: "subcategories", icon: FaBoxes, title: "Subcategories", color: "bg-[#9333EA]", linkTo: null },
        { id: "recent", icon: FaClipboardList, title: "View Recent Applications", color: "bg-[#F97316]", onClick: () => navigate("/Recentapplications") }
    ];
        ...
        const fetchAllData = async () => {
            ...
            try {
                ...
                const promises = [
                    ...
                    axios.get("https://mazedakhale.in:3000/required-documents")
                ];

                // Wait for all promises to resolve
                const [countsData, categoriesResponse, subcategoriesResponse, csCountsResponse, requiredDocsResponse] = await Promise.all(promises);
                ...
                const userAssignedDocs = requiredDocs.filter(doc =>
                    doc.user_id === parseInt(currentUserId) || doc.user_id === currentUserId
                );
                ...
                userAssignedDocs.forEach(doc => {
                    ...
                    if (category && !userCategories.some(uc => uc.category_id === category.category_id)) {
                        userCategories.push({
                            ...category,
                                        ...
                                        {category.subcategories
                                            ...
                                            .map((subcategory) => {
                                                const subcategoryCount = subcategoryCounts.find(
                                                    (count) => count.subcategoryId === subcategory.subcategory_id
                                                )?.pendingCount || "0";
